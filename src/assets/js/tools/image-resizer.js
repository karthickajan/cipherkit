/**
 * CipherKit — Image Resizer v2 (Canvas API)
...
(function(){
  ...
  +'<div class="ir-panel" id="p-crop"><div class="ir-aspect-row" id="ir-aspects"><button class="ir-aspect-btn active" data-a="0">Free</button><button class="ir-aspect-btn" data-a="1">1:1</button><button class="ir-aspect-btn" data-a="1.7778">16:9</button><button class="ir-aspect-btn" data-a="1.3333">4:3</button><button class="ir-aspect-btn" data-a="1.5">3:2</button><button class="ir-aspect-btn" data-a="0.5625">9:16</button></div><div class="ir-crop-container" id="ir-crop-box"><img id="ir-crop-img" class="ir-crop-img"><div class="ir-crop-overlay" id="ir-crop-overlay"><div class="ir-crop-frame" id="ir-crop-frame"><div class="ir-crop-handle tl" data-h="tl"></div><div class="ir-crop-handle tr" data-h="tr"></div><div class="ir-crop-handle bl" data-h="bl"></div><div class="ir-crop-handle br" data-h="br"></div></div></div><div class="ir-crop-thumb" id="ir-crop-thumb"><canvas id="ir-crop-thumb-c" width="80" height="80"></canvas></div></div><div class="ir-zoom-row"><label>Zoom</label><input type="range" id="ir-zoom" min="100" max="400" value="100"><span class="ir-zval" id="ir-zoom-val">100%</span></div><button type="button" class="pill-btn" id="ir-crop-reset" style="margin-bottom:10px">Reset</button></div>'
  ...
  function initCrop(){
    ...
    setTimeout(function(){
      ...
      _cropX=0;_cropY=0;_cropW=dw;_cropH=dh;updateCropFrame();updateCropThumb();
    },100);
  }
  document.querySelectorAll('#ir-aspects .ir-aspect-btn').forEach(function(b){b.addEventListener('click',function(){
    ...
    _aspect=parseFloat(b.dataset.a);if(_aspect>0)enforceAspect();updateCropThumb();
  });});
  ...
  function updateCropThumb(){
    if(!_img)return;var tc=$('ir-crop-thumb-c'),ctx=tc.getContext('2d');var dw=cropImg.offsetWidth,dh=cropImg.offsetHeight;if(!dw)return;
    var sx=_cropX/dw*_origW/_zoom,sy=_cropY/dh*_origH/_zoom,sw=_cropW/dw*_origW/_zoom,sh=_cropH/dh*_origH/_zoom;
    tc.width=80;tc.height=80;ctx.clearRect(0,0,80,80);try{ctx.drawImage(_img,sx,sy,sw,sh,0,0,80,80);}catch(e){}
  }
  cropFrame.addEventListener('mousedown',startDrag);cropFrame.addEventListener('touchstart',startDrag,{passive:false});
  function startDrag(e){e.preventDefault();var t=e.target;_dragType=t.dataset&&t.dataset.h?t.dataset.h:'move';var pt=getPointer(e);_dragStartX=pt.x;_dragStartY=pt.y;_startCrop={x:_cropX,y:_cropY,w:_cropW,h:_cropH};document.addEventListener('mousemove',onDrag);document.addEventListener('mouseup',endDrag);document.addEventListener('touchmove',onDrag,{passive:false});document.addEventListener('touchend',endDrag);}
  ...
  function onDrag(e){e.preventDefault();var pt=getPointer(e),dx=pt.x-_dragStartX,dy=pt.y-_dragStartY,maxW=cropImg.offsetWidth*_zoom,maxH=cropImg.offsetHeight*_zoom;
    ...
    updateCropFrame();updateCropThumb();}
  function endDrag(){document.removeEventListener('mousemove',onDrag);document.removeEventListener('mouseup',endDrag);document.removeEventListener('touchmove',onDrag);document.removeEventListener('touchend',endDrag);}
  $('ir-zoom').addEventListener('input',function(){_zoom=parseInt(this.value)/100;$('ir-zoom-val').textContent=this.value+'%';cropImg.style.transform='scale('+_zoom+')';var maxW=cropImg.offsetWidth*_zoom,maxH=cropImg.offsetHeight*_zoom;_cropW=Math.min(_cropW,maxW);_cropH=Math.min(_cropH,maxH);_cropX=Math.min(_cropX,maxW-_cropW);_cropY=Math.min(_cropY,maxH-_cropH);updateCropFrame();updateCropThumb();});
