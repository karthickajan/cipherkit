...
(function () {
  ...
  root.innerHTML =
    ...
    +     '<div class="field"><div class="field-hdr"><label for="t-input">XML Input</label><div class="field-btns"><button type="button" class="pill-btn" id="btn-clr" aria-label="Clear">' + IC.trash + ' <span>Clear</span></button></div></div><textarea id="t-input" placeholder="Paste XML here\u2026" rows="8" class="mono"></textarea><div class="inline-error" id="t-err" role="alert"></div></div>'
    +     '<button type="button" class="act-btn act-purple" id="btn-conv" aria-label="Convert to JSON">' + IC.swap + ' <span>Convert</span></button>'
    +     '<div class="out-box"><div class="out-head"><div class="out-label">' + IC.play + ' <span>JSON Output</span></div><button type="button" class="copy-btn" id="btn-cp" aria-label="Copy">' + IC.copy + ' <span>Copy</span></button></div><pre class="out-body mono ph" id="t-result" role="status" aria-live="polite">JSON output will appear here\u2026</pre></div>'
    +   '</div>'
    + '</div>'
    + '</div>';
