(function(){
  ...
  (function(){
    ...
    function showFileInfo(f){zone.classList.add('has-file');zone.innerHTML='<input type="file" id="t-file" accept="image/*" hidden><svg viewBox="0 0 24 24" fill="none" stroke="#00ff88" stroke-width="2.5" style="width:24px;height:24px;margin-bottom:8px"><polyline points="20 6 9 17 4 12"/></svg><p style="color:#00ff88;font-size:14px;margin:0">'+f.name+'</p><p style="color:#888;font-size:12px;margin:4px 0 0">'+(f.size/1024).toFixed(1)+' KB</p>';zone.style.borderColor='#00ff88';zone.style.borderStyle='solid';zone.style.background='rgba(0,255,136,.05)';if(typeof CK!=='undefined'&&CK.toast)CK.toast('Image loaded');}
  })();
  ...
  $('btn-clr').addEventListener('click',function(){var z=$('ie-drop');z.classList.remove('has-file');z.style.borderColor='';z.style.borderStyle='';z.style.background='';z.innerHTML='<input type="file" id="t-file" accept="image/*" hidden><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#555" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg><p class="upload-label">Drop your image here</p><p class="upload-sub">or <span class="upload-link">browse to upload</span></p><p class="upload-formats">Supports: PNG, JPG, WebP, GIF</p>';$('t-result').innerHTML='<span style="color:var(--muted);font-style:italic">Enhanced image will appear here\u2026</span>';_dataUrl='';var fi=$('t-file');z.addEventListener('click',function(e){if(e.target===fi)return;fi.click();});fi.addEventListener('change',function(){if(fi.files[0]){var f=fi.files[0];z.classList.add("has-file");z.innerHTML='<input type="file" id="t-file" accept="image/*" hidden><svg viewBox="0 0 24 24" fill="none" stroke="#00ff88" stroke-width="2.5" style="width:24px;height:24px;margin-bottom:8px"><polyline points="20 6 9 17 4 12"/></svg><p style="color:#00ff88;font-size:14px;margin:0">'+f.name+'</p><p style="color:#888;font-size:12px;margin:4px 0 0">'+(f.size/1024).toFixed(1)+' KB</p>';z.style.borderColor="#00ff88";z.style.borderStyle="solid";z.style.background="rgba(0,255,136,.05)";if(typeof CK!=="undefined"&&CK.toast)CK.toast("Image loaded");}});});
