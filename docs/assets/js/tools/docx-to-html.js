...
(function(){
  ...
  sty.textContent='.d2h-tabs{display:flex;gap:0;margin-bottom:12px}.d2h-tab{background:#1e1e1e;border:1px solid #333;padding:8px 18px;cursor:pointer;color:#ccc;font-family:inherit;font-size:.85rem;transition:all .15s}.d2h-tab:first-child{border-radius:6px 0 0 6px}.d2h-tab:last-child{border-radius:0 6px 6px 0}.d2h-tab.active{background:#00ff88;color:#0a0a0a;font-weight:600;border-color:#00ff88}.d2h-tab:hover:not(.active){background:#282828}.d2h-preview{background:#fff;color:#000;padding:16px;border-radius:4px;max-height:320px;overflow-y:auto;resize:vertical}.d2h-source{max-height:320px;overflow-y:auto;resize:vertical;white-space:pre-wrap;word-break:break-all;font-size:.82rem;line-height:1.6;padding:12px}';
  ...
  function syntaxHL(html){
    ...
    return e.replace(/&lt;(\/?[a-zA-Z][a-zA-Z0-9]*)((?:\s+[^&]*?)?)&gt;/g,function(m,tag,attrs){
      var hAttrs=attrs.replace(/([a-zA-Z-]+)(=)(&quot;[^&]*?&quot;|&apos;[^&]*?&apos;|\S+)/g,'<span style="color:#00ff88">$1</span>$2<span style="color:#ffd700">$3</span>');
